...
.controller('OrderFormController', function($scope, $ionicPlatform, $state, $stateParams, TimeService, $localStorage, OrderService, StatusService) {
  ...
  $scope.renamePaidStatus = function() {
    if ($scope.order.order_paid) {
      $scope.order.order_paid_text = 'Paid'
    } else {
      $scope.order.order_paid_text = 'Not-Paid'
    }
  }
  ...
  $scope.createOrder = function(orderData) {
    ...
    orderData.order_message = orderData.message+' '+orderData.cake_message;
    ...
    delete (orderData.message);
    delete (orderData.order_processed)
